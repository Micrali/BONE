...
**BoneVibAuth** 是一个依据作品书《BoneVibAuth - 利用头部振动反馈的隐式身份验证系统》实现的完整真实项目。系统以骨传导耳机为交互入口，通过播放短时 Chirp 激励信号并采集用户头部振动反馈，提取具有个体差异的 HCR 生物特征，最终结合 MFCC 特征、频响特征与 Siamese 认证模型完成低打扰、隐式、可持续的身份认证。
